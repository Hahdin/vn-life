  ...
  methods: {
    ...
    drawSparkles() {
      ...
      this.sparks.forEach((spark,i) => {
        ...
        this.ctx.fillStyle =  thisRand < 0.2 ? `rgba(255,0,0,${op})` : thisRand < 0.4 ?  `rgba(255,255,0,${op})`: thisRand < 0.6 ?  `rgba(255,125,0,${op})` : `rgba(255,255,255,${op})`; 
        let sz = 3 * spark.life / spark.lifeMax;
        this.ctx.beginPath();
        this.drawSphere(this.ctx, {x:spark.pos.x, y:spark.pos.y}, sz * thisRand , true, false);
        this.ctx.lineWidth = thisRand * 0.2;
        this.ctx.strokeStyle = thisRand < 0.2 ? `rgba(255,0,0,${op})` : thisRand < 0.4 ?  `rgba(255,255,0,${op})`: thisRand < 0.6 ?  `rgba(255,125,0,${op})` : `rgba(255,255,255,${op})`; 
        ...
        this.lineTo(this.ctx, spark.pos.x - spikeCos, spark.pos.y + spikeSin, spark.pos.x + spikeCos, spark.pos.y - spikeSin,);
        let sin = Math.sin(spark.angle * Math.PI / 180) * sz * spark.speed;
        let cos = Math.cos(spark.angle * Math.PI / 180) * sz * spark.speed;
        spark.pos.x += cos;
